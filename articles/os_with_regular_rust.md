...
タイトルで普通のRustと書きましたが、普通のRustとはどのようなものでしょうか？
本記事では下記の条件を満たす環境で実装できる状態を普通のRustと呼んでいます。
...
コードにすると以下のような感じですね。
vecを使ってhello worldしていて普通というには半ば強引ですが。。。
...
また、先程挙げた4つ以外にもOSが提供していた機能は多くありこれらを実現しても「普通の」とは程遠いかもしれませんが、今回はこの4つを実現することを目標にしていきます。
...
また、ビルドターゲットをRISC-V向けにするために.tomlファイルを用意します。これで`cargo build`実行時にターゲットを指定する手間を省くことができます。
...
今回のプログラムが動作する前にSBIというソフトウェアが動作します。SBIはデバッグコンソールへの入出力やタイマーなどの機能を提供します。

SBIは0x80200000番地に配置されたプログラムを実行するようになっているため、0x80200000番地をエントリーポイントとして定義しています。
...
これで`cargo run`を実行するとQEMUが起動し、main関数が動作するようになりました！
...
しかし悲しいことにスタックポインタは下位アドレスに伸びていきますが初期値が0なので減算することが出来ません。
よってスタックポインタの初期値を設定する必要があります。
...
サイズは4MBとりましたが、これは適当な値なので必要に応じて変更してください。
...
先程説明を省いたインライン展開についてですが、あれは関数をインライン展開し領域確保をしないための設定でした。
